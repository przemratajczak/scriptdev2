...
instance_violet_hold::instance_violet_hold(Map* pMap) : ScriptedInstance(pMap) ,
     m_bDehydrationAchievFailed(false), m_bVoidDanceAchievFailed(false)
{
    Initialize();
}
...
void instance_violet_hold::SetData(uint32 uiType, uint32 uiData)
{
    switch(uiType)
    {
        ...
        case TYPE_ICHORON:
            ...
            {
                bIsInBoss = true;
                m_bDehydrationAchievFailed = false;
            }
            break;
        case TYPE_XEVOZZ:
        ...
        case TYPE_ZURAMAT:
            ...
            {
                bIsInBoss = true;
                m_bVoidDanceAchievFailed = false;
            }
            break;
        case TYPE_CYANIGOSA:
...
bool instance_violet_hold::CheckAchievementCriteriaMeet(uint32 uiCriteriaId, Player const* pSource, Unit const* pTarget, uint32 uiMiscValue1 /* = 0*/)
{
    switch (uiCriteriaId)
    {
        ...
        case ACHIEV_DEHYDRATION:
            return !m_bDehydrationAchievFailed;
        case ACHIEV_VOID_DANCE:
            return !m_bVoidDanceAchievFailed;
        default:
            return false;
    }
}
