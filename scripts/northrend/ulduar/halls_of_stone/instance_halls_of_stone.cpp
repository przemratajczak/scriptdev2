...
struct MANGOS_DLL_DECL instance_halls_of_stone : public ScriptedInstance
{
    ...
    void OnCreatureCreate(Creature* pCreature)
    {
        ...
        {
            case NPC_KRYSTALLUS:
                m_uiKrystallusGUID = pCreature->GetObjectGuid();
                break;
            case NPC_GRIEF:
                m_uiGriefGUID = pCreature->GetObjectGuid();
                break;
            case NPC_BRANN:
                m_uiBrannGUID = pCreature->GetObjectGuid();
                break;
            case NPC_SJONNIR:
                m_uiSjonnirGUID = pCreature->GetObjectGuid();
                break;
            case NPC_KADDRAK:
                m_uiKaddrakGUID = pCreature->GetObjectGuid();
                break;
            case NPC_ABEDNEUM:
                m_uiAbedneumGUID = pCreature->GetObjectGuid();
                break;
            case NPC_MARNAK:
                m_uiMarnakGUID = pCreature->GetObjectGuid();
                break;
        }
    }
    ...
    void OnObjectCreate(GameObject* pGo)
    {
        ...
        {
            case GO_GRIEF_DOOR:
                m_uiGriefDoorGUID = pGo->GetObjectGuid();
                if (m_auiEncounter[0] != DONE)
                    CloseDoor(m_uiGriefDoorGUID);
                    else OpenDoor(m_uiGriefDoorGUID);
                break;
            case GO_BRANN_DOOR:
                m_uiBrannDoorGUID = pGo->GetObjectGuid();
                if (m_auiEncounter[1] != DONE)
                    CloseDoor(m_uiBrannDoorGUID);
                    else OpenDoor(m_uiBrannDoorGUID);
                break;
            case GO_SJONNIR_DOOR:
                m_uiSjonnirDoorGUID = pGo->GetObjectGuid();
                if (m_auiEncounter[2] != DONE)
                    CloseDoor(m_uiSjonnirDoorGUID);
                    else OpenDoor(m_uiSjonnirDoorGUID);
                break;
            case GO_TRIBUNAL_CONSOLE:
                m_uiGoTribunalConsoleGUID = pGo->GetObjectGuid();
                break;
            case GO_TRIBUNAL_CHEST:
                if (Regular) m_uiGoTribunalChestGUID = pGo->GetObjectGuid();
                break;
            case GO_TRIBUNAL_CHEST_H:
                if (!Regular) m_uiGoTribunalChestGUID = pGo->GetObjectGuid();
                break;
            case GO_TRIBUNAL_SKY_FLOOR:
                m_uiGoTribunalSkyFloorGUID = pGo->GetObjectGuid();
                break;
            case GO_KADDRAK:
                m_uiGoKaddrakGUID = pGo->GetObjectGuid();
                break;
            case GO_ABEDNEUM:
                m_uiGoAbedneumGUID = pGo->GetObjectGuid();
                break;
            case GO_MARNAK:
                m_uiGoMarnakGUID = pGo->GetObjectGuid();
                break;
        }
    }
